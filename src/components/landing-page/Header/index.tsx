        ...
        <>
            <header
                className={`header top-0 left-0 z-40 flex w-full items-center bg-transparent ${sticky
                    ? "!fixed !z-[9999] !bg-primary !bg-opacity-80 shadow-sticky backdrop-blur-sm !transition dark:!bg-color dark:!bg-opacity-20"
                    : "absolute"
                    }`}
            >
                    ...
                    <div className="relative -mx-4 flex items-center justify-between">
                        <div className="max-w-full px-4 xl:mr-12">
                            ...
                            >
                                <div className="flex justify-center items-center px-4">
                                    <Image
                                        src="/logos/opf-wbg.png"
                                        alt="logo"
                                        width={60}
                                        height={30}
                                        className="px-1"
                                    />
                                    <h1 className="font-bold text-lg md:text-3xl">OpenPathFinder</h1>
                                </div>
                            </Link>
                        </div>
                        <div className="flex w-full items-center justify-between px-4">
                            <div>
                                ...
                                <nav
                                    id="navbarCollapse"
                                    className={`navbar absolute right-0 z-30 w-[250px] rounded border-[.5px] border-body-color/50 bg-[var(--primary-color-2)] py-4 px-6 duration-300 dark:border-body-color/20 bg-primary-2 lg:visible lg:static lg:w-auto lg:border-none lg:!bg-transparent lg:p-0 lg:opacity-100 ${navbarOpen
                                            ...
                                            <li key={menuItem.id} className="group relative">
                                                {menuItem.path ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <div
                                                            className={`submenu relative top-full left-0 rounded-md bg-primary bg-opacity-60 shadow-sticky backdrop-blur-sm transition-[top] duration-300 lg:invisible lg:absolute lg:top-[110%] lg:block lg:w-[250px] lg:p-4 lg:shadow-lg lg:group-hover:visible lg:group-hover:top-full pl-4 ${openIndex === index ? "block" : "hidden"
                                                                }`}
                                                        >
                                                            {menuItem.submenu.map((submenuItem: any) => (
                                                                <Link
                                                                    ...
                                                                    key={submenuItem.id}
                                                                    className="block rounded py-2.5 text-white text-sm hover:opacity-70 lg:px-3"
                                                                >
                                                                    {submenuItem.title}
                                                                </Link>
                                                            ))}
